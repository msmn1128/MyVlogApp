...
/**
 * ひとことを自動で折り返す幅（キャンバス上のpt）。中央に置くので、左右に(1920-この幅)/2ずつ空く。
 *
 * 右端の撮影時刻（[TIME_MARGIN_PT]＋「22:31」の幅で約200pt）に重ならないよう、左右とも260pt空けている。
 * 折り返さなかった頃は、長いひとことが撮影時刻に重なり、さらに長いと画面の外で切れていた。
 * 撮影時刻の文字の大きさや余白を変えたら、ここも見直すこと。
 */
const val HITOKOTO_WRAP_WIDTH_PT = 1400f

/**
 * プレビューのみに掛かる文字の拡大率。
 * 1.0f が書き出し結果と同じ見え方。編集中に読みづらい場合は 1.2f〜1.5f に上げる。
 * 書き出される動画は変わらない。
 * ただしプレビューのひとことは書き出しと同じ位置（[HITOKOTO_WRAP_WIDTH_PT]）で折り返すので、上げると
 * 行の幅だけが広がり、撮影時刻に重なったり左右が切れて見えたりするようになる。
 */
