...
//
// 画面の縦の配分は比率（weight）の固定値だが、中身の高さは画面の比率と文字サイズで変わる。
// 固定値のままだと、端末の文字サイズを大きくしたときに見出しやタイルの文字だけが背を伸ばし、
// 波形が欄の下へ押し出されて、欄をスクロールしないと出てこなかった（文字1.3倍で確認）。
// 既定の文字サイズでも、Foldを開いた内側の画面（正方形に近い）の縦向きでは欄が足りず、
// 波形の下半分が隠れていた（中身と余白の合計719pxに対し、欄は662px）。
//
